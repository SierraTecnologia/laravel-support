...
@if($db->action == 'update')
    @section('page_title', __('support::database.editing_table', ['table' => $db->table->name]))
@else
    @section('page_title', __('support::database.create_new_table'))
@endif
...
@section('page_header')
    <h1 class="page-title">
        ...
        @if($db->action == 'update')
            {{ __('support::database.editing_table', ['table' => $db->table->name]) }}
        @else
            {{ __('support::database.create_new_table') }}
        @endif
    </h1>
@stop
...
<ol class="breadcrumb hidden-xs">
    ...
    <li>
        <a href="{{ route('facilitador.database.index') }}">
            {{ __('support::generic.database') }}
        </a>
    </li>

    @if($db->action == 'update')
    <li class="active">{{ __('support::generic.edit') }}</li>
    <li class="active">{{ $db->table->name }}</li>
    @else
    <li class="active">{{ __('support::generic.add') }}</li>
    @endif
</ol>
@endsection
...
@section('javascript')
    @include('support::tools.database.vue-components.database-table-editor')
