...
@section('page_title', __('support::generic.view').' '.$dataType->getTranslatedAttribute('display_name_singular'))

@section('page_header')
    <h1 class="page-title">
        <i class="{{ $dataType->icon }}"></i> {{ __('support::generic.viewing') }} {{ ucfirst($dataType->getTranslatedAttribute('display_name_singular')) }} &nbsp;
        ...
        @can('edit', $dataTypeContent)
            <a href="{{ \Support\Routing\UrlGenerator::managerRoute($dataType->slug, 'edit', $dataTypeContent->getKey()) }}" class="btn btn-info">
                ...
                {{ __('support::generic.edit') }}
            </a>
        @endcan
        @can('delete', $dataTypeContent)
            @if($isSoftDeleted)
                <a href="{{ \Support\Routing\UrlGenerator::managerRoute($dataType->slug, 'restore', $dataTypeContent->getKey()) }}" title="{{ __('support::generic.restore') }}" class="btn btn-default restore" data-id="{{ $dataTypeContent->getKey() }}" id="restore-{{ $dataTypeContent->getKey() }}">
                    <i class="facilitador-trash"></i> <span class="hidden-xs hidden-sm">{{ __('support::generic.restore') }}</span>
                </a>
            @else
                <a href="javascript:;" title="{{ __('support::generic.delete') }}" class="btn btn-danger delete" data-id="{{ $dataTypeContent->getKey() }}" id="delete-{{ $dataTypeContent->getKey() }}">
                    <i class="facilitador-trash"></i> <span class="hidden-xs hidden-sm">{{ __('support::generic.delete') }}</span>
                </a>
            @endif
        @endcan
        ...
        <a href="{{ \Support\Routing\UrlGenerator::managerRoute($dataType->slug, 'index') }}" class="btn btn-warning">
            ...
            {{ __('support::generic.return_to_list') }}
        </a>
    </h1>
                    ...
                    @foreach($dataType->readRows as $row)
                        ...
                        <div class="panel-body" style="padding-top:0;">
                            ...
                            @elseif($row->type == 'relationship')
                                 @include('support::components.forms.fields.relationship', ['view' => 'read', 'options' => $row->details])
                ...
                <div class="modal-footer">
                    <form action="{{ \Support\Routing\UrlGenerator::managerRoute($dataType->slug, 'index') }}" id="delete_form" method="POST">
                        ...
                        <input type="submit" class="btn btn-danger pull-right delete-confirm"
                               value="{{ __('support::generic.delete_confirm') }} {{ strtolower($dataType->getTranslatedAttribute('display_name_singular')) }}">
                    </form>
